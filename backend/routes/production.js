...
import { createProductionJournal } from "../utils/accounting.js";
...
router.get("/", (req, res) => {
  ...
  const orders = db.prepare(
    "SELECT id FROM production_orders ORDER BY id DESC"
  ).all();


  res.json(
    orders.map(o => getOrder(o.id))
  );
...
router.post("/", (req, res) => {
  ...
  res.status(201)
    .json(getOrder(result.lastInsertRowid));
...
router.post("/:id/complete", (req,res)=>{
  ...
  if(!order)
    return res.status(404)
      .json({error:"أمر الإنتاج غير موجود"});



  if(order.status==="COMPLETED")
    return res.status(400)
      .json({error:"تم تنفيذ الأمر مسبقاً"});
